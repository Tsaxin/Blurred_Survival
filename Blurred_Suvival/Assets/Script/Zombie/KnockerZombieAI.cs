    ...
    TileData FindChasingTileAndAttackOpportunity(out CharacterStats attackTarget)
    {
        ...
        for (int dy = -moveRange; dy <= moveRange; dy++)
        {
            for (int dx = -moveRange; dx <= moveRange; dx++)
            {
                ...
                if (row < 0 || row >= tileManager.tiles.GetLength(0) ||
                    col < 0 || col >= tileManager.tiles.GetLength(1)) continue;
                ...
                int dxToPlayer = Mathf.Abs(pos.x - playerIndex.x);
                int dyToPlayer = Mathf.Abs(pos.y - playerIndex.y);
                if (Mathf.Max(dxToPlayer, dyToPlayer) == 1)
                {
                    attackTarget = nearestPlayer;
                    return tile;
                }

                if (distFromCurrent == 1) allAdjacentBlocked = false;
            }
        }

        if (movableTiles.Count == 0 || allAdjacentBlocked) return null;
